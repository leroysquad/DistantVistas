...
/// <summary>
/// Server half of the optional assist (DESIGN.md Â§10). Stage 1 answers the handshake and
...
public class LodAssistServerSystem : ModSystem
{
    ...
    public override void StartServerSide(ICoreServerAPI api)
    {
        ...
        api.ChatCommands.Create("dvserver")
            .WithDescription("DistantVistas server assist status")
            ...
            .HandleWith(_ =>
            {
                ...
                return TextCommandResult.Success(
                    $"[DistantVistas] {config.Describe()}. Cache: {capture?.SectionCount ?? 0} sections, "
                    ...
                    + "Settings live in ModConfig/distantvistas-server.json (restart to apply).");
            });

        Mod.Logger.Notification(
            "DistantVistas {0} server assist listening. Players without the mod are "
    ...
    {
        Mod.Logger.Debug("DistantVistas: assist hello from {0} (client {1}, protocol {2})",
        ...
        sapi.Event.EnqueueMainThreadTask(() => Answer(fromPlayer), "distantvistas-hello");
    }
    ...
    /// client ever got. An admin running /dvgen while people were online therefore built
    ...
    void OfferNewKeys()
    {
        ...
        Mod.Logger.Debug("DistantVistas: offered {0} newly cached sections to {1} player(s)",
            delta.Length, told);
    }
    ...
    void SendManifest(IServerPlayer player, long[] keys)
    {
        ...
        Mod.Logger.Debug("DistantVistas: sent {0} keys to {1} in {2} chunks",
