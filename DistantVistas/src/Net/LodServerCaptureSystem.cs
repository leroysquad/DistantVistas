...
public class LodServerCaptureSystem : ModSystem
{
    ...
    void MaybeStartDeferredSweep()
    {
        ...
        if (Config.AutoGenerateOnJoin && autoGeneratedPlayers.Count == 0) return;
    ...
    {
        if (!Config.AutoGenerateOnJoin || !Config.EnableCapture || !Config.GenerateEnabled) return;
        if (!worldReady) return;
        if (generate is { Done: false }) return;

        string uid = byPlayer.PlayerUID ?? byPlayer.PlayerName ?? "?";
        if (!autoGeneratedPlayers.Add(uid)) return;

        var pos = byPlayer.Entity?.Pos;
        if (pos == null) return;

        int radius = Config.AutoGenerateRadiusChunks > 0
            ? Config.AutoGenerateRadiusChunks
            : Config.GenerateDefaultRadiusChunks;
        if (radius <= 0) return;
        ...
        generate = new LodPlayerPregen(sapi, Mod.Logger, OpenPipeline(),
            centreCx, centreCz, radius, Config, "auto-join:" + (byPlayer.PlayerName ?? uid),
            columnsPerSecond: Config.GenerateColumnsPerSecond, skipExistingLoads: true);
        generate.Start();
        ...
        Mod.Logger.Notification(
            ...
            byPlayer.PlayerName ?? uid, radius, columns, time);
    }
