...
public sealed class LodAssistClient
{
    ...
    public void Greet()
    {
        ...
        if (!channel.Connected)
        {
            ...
            Status = "none (server does not have DistantVistas)";
            logger.Debug("DistantVistas: no server assist (channel state {0})",
        ...
        catch (Exception e)
        {
            ...
            logger.Warning("DistantVistas: server assist handshake failed, continuing without it: {0}", e);
        }
    }
    ...
    internal void OnWelcome(AssistWelcome msg)
    {
        ...
        if (!msg.Enabled)
        {
            ...
            Status = reason.Length > 0
                ? $"server has DistantVistas but the assist is off ({reason})"
                : "server has DistantVistas but the assist is off";
            logger.Notification("DistantVistas: {0}", Status);
            return;
        }
        ...
        if (negotiated < 1)
        {
            ...
            logger.Warning("DistantVistas: {0}", Status);
            return;
        }

        NegotiatedProtocol = negotiated;
        Status = $"connected to server {version} (protocol {negotiated})";
        logger.Notification("DistantVistas: server assist {0}", Status);
    }
    ...
    public long[] Request(IEnumerable<long> wanted)
    {
        ...
        catch (Exception e)
        {
            ...
            logger.Warning("DistantVistas: section request failed: {0}", e);
    ...
    public void Pump(LodForeignSectionInstaller install)
    {
        while (manifestChunks.TryDequeue(out (long[] Keys, bool Last) chunk))
        {
            ...
            if (ManifestComplete)
            {
                logger.Debug("DistantVistas: server offered more sections, {0} now known",
                    RemoteKeys.Count);
                continue;
            }
            ...
            logger.Notification(
                "DistantVistas: server key manifest complete - {0} keys received{1}",
                RemoteKeys.Count,
