## 1.0.35
- **Login overlay speed cut (GC + GetColor hot path).** 1.0.34 fixed scout FIFO stall (`paintReadyQueued>0`, L0 past ~358) but Sovereign playtest still showed ~9 GB managed / 2383 gen0 in first 30s, avg ~540 GetColorCalls/batch, 200ms paint spikes, and 16k inline loads competing with bake. 1.0.35 targets real throughput: **16×16** GetColor cache tile (was 8×8), **BlockId-only** cache for climate-untinted blocks, **season-rel tile cache**, no partial **palette snapshot** rebuild during chunked paint, paint wall **120ms** with **32** stops/tick, **spawn-near-first** paint queue + revisit budgeting (75% inner disk), overlay inline storage installs capped at **2/tick @ 1ms**, throttled resume snapshots, scout chunk wait **96** ticks. Details + verify signals: `docs/plans/login-bake-walltime-1033.md` § 1.0.35.
...
- **Verify:** first-30s gen0 much lower; H-PAINT `getColorCalls` avg &lt;250; spawn neighbourhood FlagBaked before rim; `paintReadyQueued` healthy; no star jumps.
