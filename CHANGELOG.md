## 0.8.8
- **CRITICAL: login visit sweep blind teleports (0.8.7).** Warmup advanced after one pulse even when no loading cover painted, so ~43 quiet teleports flashed sky/terrain on joins with an empty `CachedScreens` loading screen. Fix: gate visit teleports on `IsOverlayReady` — eight consecutive ortho opaque frames from the stock fallback, or eight successful vanilla `RenderToDefaultFramebuffer` paints. If the cover never becomes ready within `OverlayWarmupMaxTicks` (20), abort cleanly into play instead of blind-teleporting. Esc cancel and resume checkpoint unchanged.
- **Stock fallback draw hardening.** `PrepareImmediate` pre-warms the white tint texture and owned quad mesh; solid-colour draws try explicit quad, then ortho internal-quad tint, then baked-colour internal quad; transient zero-sized frames no longer reset the health counter.

## 0.8.7
- **CRITICAL: pulse starvation when vanilla draw blocks (0.8.6).** `OnRenderPulse` still ran on the Ortho pass immediately before `RenderToDefaultFramebuffer`, so an async-sound block on that draw prevented further frames and starved sweep ticks after the first pulse. Fix: pulse **only** on Ortho (no vanilla draw); draw on `AfterFinalComposition` only so each frame's Ortho pulse always runs first. Repeating `RegisterCallback(50)` backup while the sweep is active.
- **Bootstrap planning freeze.** Empty-canvas bootstrap still enumerated the full ~27k-cell disk synchronously in `Begin()` and called `ClassifyCell` on every cell before the first teleport. Fix: sort disk cells by spawn distance and cap synchronous rain-height probes to `BootstrapMaxProbeClassifications=2048` (nearest cells only); coast-guard falls back to radius mode when the partial probe is inconclusive.
- **Uncapped season revisit.** `PlanSweepQueue` queued every visited L0 key (10k+ on large saves) with no per-login budget. Fix: apply the same `BudgetVisitStops` cap (`RevisitMaxVisitStops`, 43 by default) for season refresh; remaining keys resume on relog via the existing checkpoint file.
- **LoadAndCacheScreen removed.** `FindCachedLoadingScreen` scans `CachedScreens` only; the `LoadAndCacheScreen` fallback is gone because it can construct a fresh screen and re-trigger `OnScreenLoaded` / async sound loading.
- **Gate miss reuse.** `LodLoginSweepGate` passes its audited miss list (including empty) into `LodLoginBake.Begin` so `PlanSweepQueue` never scans the entire visited canvas twice on season-revisit joins.
