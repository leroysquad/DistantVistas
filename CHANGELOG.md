...
- **LoadAndCacheScreen sound trap.** `FindCachedLoadingScreen` tried `LoadAndCacheScreen` before the `CachedScreens` dictionary, which could construct a fresh `GuiScreenLoadingGame` and re-trigger async sound loading. Fix: dictionary lookup first; `LoadAndCacheScreen` only as last resort.
- **Gate miss reuse.** `LodLoginSweepGate` now passes its miss list into `LodLoginBake.Begin` so `PlanSweepQueue` does not scan the entire visited canvas twice on join.
