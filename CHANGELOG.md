## 0.7.16
- **Stable full-detail terrain, incomplete mountain fragments, and moving grass band.** Existing L0 captures are preserved while old compressed parent levels are rebuilt, but visible selection now stays at L0 at every distance: backing away cannot swap a loaded mountain for a blocky approximation. Partial L0 sections remain hidden until every column is captured, so unknown terrain appears empty instead of becoming disconnected shelves, pillars, or skinny islands. Cached and integrated-server palette colours refresh through the current stable topsoil composite, fixing old brown grass that turned green only when vanilla chunks replaced the LOD. Isolated test first; public update after in-game signoff.
