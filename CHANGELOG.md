## 1.0.32
- **Login bake speed (A-tier) plus GetColor GC cuts (B-tier).** 1.0.31 already runs 16 scouts in parallel (8 near mesh-wait / 8 far FlagBaked-release) and paints `scoutReady` each tick instead of one serial `BakeBatchAtStop`. 1.0.32 folds the research-branch micro-wins and the remaining ranked bottlenecks: thread-local `LodBakeScratch` `BlockPos` + `ArrayPool` column arrays, **per-section texture-mean cache** (one 8-sample `GetColorWithoutTint` per BlockId per L0, not ×4096 columns), reused scout `readyScratch` / in-place leftover batch keys. `MaxBakePerTick` stays **24**. Pinned scouts re-partition every **8** ticks (not every tick). Spawn reveal ring every **4** ticks. Mesh/Paint never `forceRecapture`. Sticky empty-mesh claims release the scout slot. Leftover neighbour bake is visit-cell-only past 1024. Overlay ETA uses wall/finishes (a 24-scout tick is not one 0.25s hop). Farseer visit-mask rebuilds at most every 500ms on HasDataSet churn (stamp L0 keys + envelope disk, not 256×256 `Contains`). Height enrich waits until overlay ends. Plan + citations: `docs/plans/login-bake-efficiency.md`.
