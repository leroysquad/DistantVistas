## 0.8.36
- **CRITICAL: lavender/purple grass on far LOD after leave (0.8.35 playtest).** Near vanilla was green; far DV LOD went lavender when walking away. Root cause: live-tint path applied per-channel `localCl.rgb / keepCl.rgb` on `Dilute(share, climate_keep)` slot tints — skews grass magenta when biomes differ. Fix: (1) **explore-time budgeted bake** — `LodExploreBake` queues L0 sections after capture / walk-back and drains **1–2 sections/tick** via `BakeSectionFromVisit` + `FlagBaked` while chunks are loaded (same hitch budget as capture apply); (2) **shader safety net** — spatial climate shift uses **luminance scale** (matches `LodClimateField.ApplyLocalClimate`). Login sweep ~1 min, splash opaque cover, pose restore unchanged.
