## 0.8.36
- **CRITICAL: lavender snap-back + magenta section patches (May/spring, high altitude).** Hard chunky LOD-section rectangles on hills/canopies = live-tint mesh (`Baked=false` or visit-baked rows meshed as thin band 2). **Product intent:** login visit bake captures **real live seasonal `GetColor`** (30-day window + bootstrap; 0.8.35 ~60s). Explore/discover uses the **same** `BakeSectionFromVisit` when map chunks are resident. **Fixes:** (1) visit-baked palette always meshes as **band 3** (canopy/flowers no longer stay thin band 2 live-tint); (2) visit bake **per-column** (`IsColumnMapLoaded`), not all-or-nothing 4-chunk gate; (3) `FinalizeL0DiscoverBake` + explore drain always attempt visit bake, remesh + mip burst. Spring snow-row safety on **unbaked** grass only (`lodterrain.vsh`). Splash cover-fit retained.
