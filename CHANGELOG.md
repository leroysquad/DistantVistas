## 0.8.36
- **CRITICAL: lavender/purple far LOD after leave (May/spring playtest).** Hypothesis: snow-row climate samples (white, low chroma) live-tinted onto valley grass in spring read as lavender sheets instead of green. Root cause confirmed: explore capture stored untinted composite + live tint slot (`DescribePalette` always `Baked=false`) so `albedo = vertexColor * tint` after vanilla unloads. **Primary fix:** resident map chunks → `TryDiscoverBake` samples vanilla `GetColor`, stores `FlagBaked` + `SlotNone` (band 3); non-provisional L0 capture runs `BakeSectionFromVisit` on apply (remesh, mip, persist). GetColor miss → `BakePaletteColor` with `ProtectHighTintFromSnow` / `ClampTintAwayFromWhite`. Real snow caps bake `GetColor` so spring season mix cannot lavenderize white snow. **Safety nets:** budgeted `LodExploreBake` (1–2 L0/tick), `lodterrain.vsh` snow-row high clamp + luminance climate scale (not per-channel ratio). **Also in this build:** login splash resolution-independent (`LodLoginSplashLayout.CoverFit`, tiled quads); login sweep ~60s at VD≈2000 (`TargetMaxSec=60`, ~30 stops).

## 0.8.35
- **Login sweep retarget: ~1 minute wall-clock (was 3–5 min).** `TargetMaxSec` / `BootstrapTargetMaxSec` **60s**; `InitialSecPerStop` **2.0** → `VisitStopBudget` yields **~30 land/coast teleports** (was 75 @ 300s). Efficiency: `TeleportSettle`/`BakeSettle` tightened; `MaxChunkWaitTicks` 48→**20** (~1.0s), `MaxCaptureWaitTicks` 28→**14** (~0.7s); batch neighbour bake radius **8** / cap **24** L0 cells per stop at **2048** view; open-ocean full-bake samples **3** (stamp fill unchanged). View boost floor **`SweepMinViewDistanceBlocks` = 2000** (ceiling 2048) held for the whole sweep via `EnsureBoosted` each tick. Gap-first `FilterNeedsVisit`, ocean stamp-not-full-bake, pose restore, and **0.8.34 splash opaque-cover fix** retained.

## 0.8.34
- **CRITICAL: opaque cover draw failed at valid frame size (0.8.33 playtest).** Present-path splash paint reached `DrawOpaqueCover` at 2560×1369 but never logged `first opaque frame painted`. Root cause: `PaintSweepFrame` used `orthoPass: true` + internal GUI `Render2DTexture(int,…, Vec4f)` — that tint path only works inside `EnumRenderStage.Ortho`. Harmony present hooks run outside the ortho stage, so internal quads failed; explicit `MeshRef` quads could also fail with depth test still enabled and `CurrentFrameBuffer` null (no `ClearFrameBuffer`). Fix: `PaintPresentPath` uses `orthoPass: false`, pairs `OrthoMode`/`PerspectiveMode`, calls `GLDisableDepthTest`/`GLEnableDepthTest` around the draw, resolves Primary framebuffer when `CurrentFrameBuffer` is null, and draws baked solid-color explicit quads before white/internal fallbacks. Register splash on `EnumRenderStage.Done`; LoadingGame present paints in Prefix when vanilla draw is skipped. 0.8.27–0.8.33 bootstrap/ocean/gap-first/pose-restore retained.

## 0.8.33
- **CRITICAL: black screen after char-create → bootstrap (0.8.28 playtest).** RunningGame `RenderToDefaultFramebuffer` Postfix alone was insufficient: Harmony skipped `GuiScreenLoadingGame.RenderToDefaultFramebuffer` during `SweepActive` with **no replacement paint**, which prevented Ortho/AfterFinal render stages from running (`0 meshes`, `draw submit 0.0`, never `first opaque frame painted`). Handover was also deferred whenever login-sweep was **enabled in config**, not only while armed. Fix: paint splash on **LoadingGame + RunningGame present Postfixes** and **OnNewFrame Postfix** safety path; defer handover only while `SweepActive`; `EnsureRunningGameRenderPath` completes handover to RunningGame without clearing sweep; Ortho renderer stays active while sweep armed. Splash paint failure logs frame-size/cover diagnostics. 0.8.27–0.8.32 bootstrap/ocean/gap-first/pose-restore retained.
...
## 0.8.30
- **Bootstrap ocean: expose + sample bake + stamp fill (0.8.29 correction).** Open-ocean L0 body tiles no longer get a full teleport+bake each (0.8.29 skipped them entirely → sky holes over water). Fix: up to **6 representative ocean sample visits** (spread across the body, not counted in the **75** land/coast stop budget) establish water palette from live terrain; remaining open-ocean cells get a **cheap flat stamp** (`LodLoginSweepOceanFill.StampOpenOcean`) after the visit pass so far vistas show water. Land, unknown, and coastline ocean cells still get full visit+bake. `InitialSecPerStop` restored to **4.0** → **~75 land stops / ~5 min** (same 300s wall-clock cap). 0.8.27 splash present path, 2048 view boost, inner-weighted bootstrap, batch neighbour bake, and per-world complete retained.

## 0.8.29
- **Bootstrap ocean skip + bake efficiency (0.8.28 playtest follow-up).** Open-ocean L0 tiles (no land neighbour) are excluded from first-join planning — budget goes to land + coastline only (`SelectVisitWorthyCells`; coast-guard no longer queues every ocean body cell). Per-stop dwell tightened: TeleportSettle 10→4 ticks, BakeSettle 12→3, MaxChunkWait 80→48, MaxCaptureWait 48→28 (~1.4–2.4s caps). After capture idles at a stop, **batch-bake** up to 16 planned L0 neighbours already loaded by the 2048-block column sweep (6-cell radius) instead of one bake per teleport. `InitialSecPerStop` 4→2.5 → **~100 visit stops / ~4 min** at estimate (same 300s wall-clock cap). View boost runs before the ARMED log so the first teleport sees 2048-block sweep. 0.8.27 splash + 0.8.28 inner-weighted bootstrap retained.

## 0.8.28
- **CRITICAL: first-join bootstrap too sparse (0.8.26 playtest).** `Bootstrap (new world) (38 of 27604)` finished in ~2.5 min but exposed only a tiny fringe beyond the walked frontier — 38 linear distance picks across a ~6 km / ~27k L0 probe disk is ~1 stop per long outer arc. Root cause: `BootstrapTargetMaxSec` was 150s (half the revisit window) and `BudgetVisitStops` sampled uniformly by distance rank. Fix: align bootstrap wall-clock with revisit (**300s / ~75 stops at 4s/stop**, same 3–5 min target); add `BudgetBootstrapVisitStops` — inner-weighted distance bands (~10–18 rings) with more teleports near spawn and multiple angular picks per ring so the full `EmptyCanvasBootstrapRadiusBlocks` disk gets usable vista cover. Revisit keeps linear `BudgetVisitStops`. 0.8.27 splash present path unchanged.

## 0.8.27
- **CRITICAL: black screen during login visit sweep (0.8.26 playtest).** `SuppressRunningGameRender=false` (0.8.25) let `RenderToPrimary` present, but registered Ortho/AfterFinal splash IRenderers never ran while `handOverRenderingToRunningGame` was deferred — `HasEverPaintedOpaque` stayed false, logs repeated `loading cover not painted yet`, player saw permanent black through bootstrap teleports. Root cause: splash paint was removed from the RunningGame **framebuffer present path** without a working substitute on the deferred-handover render graph. Fix: restore `PaintSplashCover` → `PaintSweepFrame` on `GuiScreenRunningGame.RenderToDefaultFramebuffer` **Postfix** when `SweepActive` (not from OnNewFrame — that path does not present). Keep `SuppressRunningGameRender=false`, WorldHide, bootstrap-first planning (0.8.26), per-world complete, ≤30-day skip, OrthoMode/PerspectiveMode pair, and async-sound LoadingGame skip unchanged.
- **HUD restore after sweep.** `LodLoginBakeHudHide` retries hide/show up to three times, tracks when the sweep forced hide, and prefers showing HUD when restore is ambiguous — reduces `HUD restore mismatch (HideGuis=True, wanted=False)` after sweep finish.

## 0.8.26
- **CRITICAL: first login refresh-only left vistas as sky (0.8.24/0.8.25 playtest).** After the black-screen sweep finished `Refreshing visited land (season) (75 of 123)` with gate reason `no successful sweep recorded yet for this world`, the player saw no new land beyond what they had already walked. Root cause: `PlanSweepQueue` / `LodLoginSweepBootstrap.Plan` treated any non-empty `VisitedL0Keys` as a season revisit — so a few walked cells before the first successful sweep selected **PlanRevisitKeys** instead of the ~6 km bootstrap disk (`EmptyCanvasBootstrapRadiusBlocks`). Fix: when there is **no per-world complete marker**, always plan **Bootstrap** (coast guard / spawn-radius) with spatially spread `BudgetVisitStops` — even if visited keys are non-empty. Season **Refreshing visited land** only after a prior complete (miss-repair, canvas growth, or outside 30-day window). Gate orders no-complete before miss-repair. 0.8.25 splash present path retained (`SuppressRunningGameRender=false`, Ortho/AfterFinal splash, WorldHide, per-world complete, ≤30-day skip).

## 0.8.25
- **CRITICAL: black screen during login visit sweep (0.8.24 playtest).** `LodLoginBakeSweepGate.Arm` set `SuppressRunningGameRender=true`, so Harmony skipped `GuiScreenRunningGame.RenderToPrimary` / post / Ortho / AfterFinalComposition. OnNewFrame `PaintSweepFrame` cleared/painted with OrthoMode but nothing presented the framebuffer — teleports ran while the player saw permanent black (`loading cover not painted yet`, never `first opaque frame painted`). Fix: **do not suppress RunningGame render** during the sweep; keep WorldHide for teleport flash; splash draws only from registered Ortho/AfterFinalComposition IRenderers so present works. Gut OnNewFrame / framebuffer-prefix OrthoMode paint path (matrix-leak risk + no present). LoadingGame async-sound skip, char-create deferral, per-world complete (0.8.24), and ≤30-day skip unchanged.
...
- **Skip window is 30 in-game days only.** `LodLoginSweepWindow.IsWithin` no longer treats same-season alone as eligible — rejoin skip / resume requires `TotalDays` gap ≤ `MaxResumeDayGap` (30). Outside that window, re-sweep even if the season slug matches.

## 0.8.22
- **CRITICAL: rejoin Loading… deadlock (0.8.21).** `LodLoginBakeCharacterWait.IsPending` gated on `!PlayerReadyFired`, which is true on almost every `LevelFinalize` before handover — so rejoins deferred the sweep forever while Harmony still blocked `handOverRenderingToRunningGame`. Fix: `IsPending` is dialog-only (`HasProtectedDialogOpen`). LevelFinalize prefers Decide/skip first; real character/class deferral immediately allows handover (`AllowHandoverWhileCharacterPending`) so vanilla UI can show, then starts the sweep once dialogs close.
- **In-window skip ignores leftover incompletes.** If `login-sweep-complete.json` exists, visited count has not grown, and the season/30-day window still holds (`MaxResumeDayGap` = 30), skip the full revisit even when `FindMisses` reports leftovers — do not re-canvas the same world in-window. Empty-canvas bootstrap, eligible mid-sweep resume, canvas growth, and outside-window still run.
...
- **View-distance boost during sweep.** Temporarily raises vanilla `DesiredViewDistance` to the server-approved max (up to 2048 blocks), clears `FarViewDistanceCap` for unlimited DV draw, and lowers `OverdrawStart` to 0.35 for wider cover. Chunk stream/sweep radius scales with boosted view distance. All settings restore on finish, Esc, or cancel — play FPS unchanged.
...
## 0.8.12
- **CRITICAL: double-assembly load / missing DistantVistas (0.8.11).** `clientsettings.json` `modPaths` listed both relative `Mods` and the absolute AppData `Mods` folder — the same zip was discovered twice. First `LoadAssembly` succeeded; the second failed with `Assembly with same name is already loaded`, the failed container poisoned the mod (no External Origins entry, no systems, no `DistantVistas 0.8.11 loaded` line), and the client stayed on vanilla `Loading…` at `Loading world map cache db...`. Fix for install: keep a **single** modPath to AppData `Mods`. Zip install only there; never also under `ClientMods` as a live origin.
- **Handover release hardened.** `ClearHandoverDeferral` clears sweep/deferral flags **before** screen attach, always best-effort sets `loadingText` to `Finishing 100%`, and still forces `LoadScreenNoLoadCall(running)` when possible even if vanilla loader attach failed earlier. LevelFinalize wraps sweep setup in try/catch so a setup exception still releases Loading….
- **Visible progress on vanilla loader.** Status lines now use phase labels **Preparing / Visiting / Finishing** with an explicit **percent** (throttled every ~5%). Driven via `ScreenManager.loadingText` (GuiScreenLoadingGame exposes `textElem` only — no stock progress-bar field to hook). Custom DV splash stays off.
