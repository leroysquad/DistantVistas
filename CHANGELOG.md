## 0.8.36
- **CRITICAL: lavender snap-back after leave (May/spring).** Hard swap at vanilla load/unload — DV mesh kept live-tint (`Baked=false`); vanilla only covered it. **Product intent:** login visit bake captures **real live seasonal colors** via `GetColor` / `BakeSectionFromVisit` (30-day window + bootstrap unchanged; 0.8.35 ~60s timing). Explore/discover must use that **same live visit bake** when chunks are resident — FlagBaked + SlotNone (band 3), not live-tint sheets or shader-repro substitutes. **Fix:** `TryDiscoverBake` → `SampleVanillaColor` only when map chunk loaded; `FinalizeL0DiscoverBake` → `BakeSectionFromVisit` + remesh/mip burst; `LodExploreBake` drains visit bake when chunks load later. Spring safety net on **unbaked** grass only: `lodterrain.vsh` snow-row clamp + luminance climate. Splash cover-fit + tiled quads retained.
