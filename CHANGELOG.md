...
- **Load + idle remesh hardening.** Persisted `FlagBaked` survives palette reclassify (no double live tint on reload). Month repaint drops the GPU mesh before `RenderDirty` so standing still still remeshes. Peak snow-band map samples fall back to sea-level climate; same blockId prefers the higher capture Y.
