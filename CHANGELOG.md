## 1.0.46
- **Queue-safe login bake past the 1.0.45 ~734 autosave death.** Replaces open-loop per-tick ceilings with completion-gated admission: server LodServerChunkRequestGate (24 in-flight priority loads, 2 starts/tick, 2 ForceSend/tick, coalesced pending, owner cancel), append-only ScoutHostStatus pressure snapshots, client LodLoginChunkRequestCoordinator (8-burst / 16/s token bucket, overlay	o background handoff), and stream growth that keeps the frontier formula but pauses under hitch or host/request pressure until 3 healthy snapshots + 5s quiet dwell. Pregen/savegame/peek producers share the same gate. Telemetry runId **1046**. Pass condition remains RequestChunkColumnsQueueSize = 2000 — no magicnumbers bump required.
- Drop distantvistas_1.0.46.zip in Mods. Do not extract. Fully quit Vintage Story, then start it again.

## 1.0.45
- **Don't kill the SP server after clearing 639 (1.0.44 autosave death).** Playtest runId **1044** climbed past 639 (`streamViewBlocks` 1184→1216, Capture alive) then the integrated server stopped: `Unable to autosave, was not able to pause the server` / `threads were not suspended: chunkdbthread` ×20 → `Server autosave failure`. `Indexed Fifo Queue overflow` on `RequestChunkColumnsQueue` (`SupplyChunks` / `loadChunkAreaBlocking`). Shutdown still saved 77064 chunks. Cause: overlay VD jumped 1024→1184 in one write, hop reset the spawn reveal disk and re-requested it, hop `RequestChunkColumnsVisible` used the full ~40-chunk stream radius, and scout `RequestUp` retries re-issued whole KeepLoaded rings into chunkdb. Fix: **step stream 32 blocks / 5s**, skip grows while overlay hitch ≥800ms; overlay `SetChunkColumnVisible` **budget 96/tick** and **spawn-solid cap** (vanilla VD covers the extra ring); hop visible = L0 neighbourhood only; **idempotent HoldAnchor** + **24 priority loads/tick**. Visit/Farseer disk still **4075**. Scouts primary; player stays at pickup. Telemetry runId **1045**. Optional safety net: raise `RequestChunkColumnsQueueSize` in `servermagicnumbers.json` while baking. Details: `docs/plans/login-bake-walltime-1033.md` § 1.0.45.
- Drop `distantvistas_1.0.45.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** `finished` past **639** toward **800+** without `Server autosave failure`; `stream-grow` steps **+32** with `dwellMs` 5000 (not 1024→1184 in one line); `hitchPressure` true during long ticks then false; `captureLive>0`; slider restores; `server-main.log` has no 20-in-a-row suspend failures.

## 1.0.44
- **Frontier-relative overlay stream (1.0.43 playtest stalled at finished≈639).** 1024 spawn-solid broke the 358/750 wall; the same run then froze at **finished=639 / pending≈1025** (~913 blocks, ~0.89 of stream 1024) with `waitChunksLive=14`, `captureLive=0`, `paintStarveTicks` climbing, hop rings 20–27 recycling `distFromPickup` **836–846** and **`residencyLoaded=0`**. Hopping CameraPos/entity off pickup fought server WorldManager (IPlayer stays at pickup). Fix: overlay vanilla / LastApproved **grows** with filled radius — `max(1024, finishedRadius+256, finishedRadius/0.88)` snapped to chunk size, cap **2048** (engine ceiling, not the 4075 visit/Farseer disk). Hop is **priority RequestUp only**; player + look + CameraPos stay at exact pickup. Annulus inner is **max(finishedRadius, 750)** so unlocks sit on the next ring, not the old 750–1024 band. Telemetry runId **1044**: `stream-grow`, `scout-budget.streamViewBlocks`. Details: `docs/plans/login-bake-walltime-1033.md` § 1.0.44.
- Drop `distantvistas_1.0.44.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** `streamViewBlocks` grows past **1024** near finished 639 (expect ~1184); `finished` past **639** toward **800+**; `captureLive>0`; `paintReadyQueued>0`; hop `distFromPickup` near filled radius (not stuck 836–846); `residencyLoaded≥1` **or** hop never fires because scouts already Capture; slider restores after overlay (not stuck at 1184/1536).

## 1.0.43
- **Cliff residency: stream spawn-solid, stop hopping as the load path (1.0.42 still `loaded=0`).** Hop targeting was already correct; player hop + extra `RequestUp` never residented map chunks. Two code facts: (1) overlay vanilla view stayed **750**, so the client view-culls ForceSent columns in the 750–1024 annulus around pickup; (2) **16/16 scout holds** meant the 1.0.42 pump was the **17th** `RequestUp` and was queued, never KeepLoaded. Camera stayed pinned at pickup every tick. Fix: write vanilla / LastApproved view to **1024** during overlay (visit/Farseer disk still **4.5×750+700=4075**); hold cap **18** + **priority evict** so the residency pump is not refused; **CameraPos follows hop** (look yaw/pitch stay pickup); **4** cold-near WaitChunks now that those cells can load. Telemetry runId **1043**. Details: `docs/plans/login-bake-walltime-1033.md` § 1.0.43.
- Drop `distantvistas_1.0.43.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** `finished` past **358** and climbing; `captureLive>0`; `paintReadyQueued>0`; `hop-residency-probe` `residencyLoaded≥1` **or** hop never fires because scouts already Capture; slider restores after overlay (not stuck 1024).
...
- **Verify:** `finished` past **358**; `hop-unlock` fires at cliff; `captureLive>0` after unlock; `paintReadyQueued>0`; pickup restore in `pickup-restore` log.

## 1.0.38
- **Warm-ring cliff fix (~358 stall).** Root cause: filled-disk progress exits the overlay **stream-hold** radius (~750 blocks) while pending keys remain inside the **spawn-solid** band (1024) — classified as near scouts on **cold-near** cells with no map chunks. Forced Capture without residency → **captureStall** hot-loop (733 stalls vs 6 painted in 1.0.37); `chunkPressure` never fired because scouts were in Capture not WaitChunks. Fix: **paint-readiness scheduling** — never force Capture on cold keys while paint starves; resident handoff in Capture; exponential captureStall cooldown; cap cold-near live scouts to half fleet; **one cold-near WaitChunks streamer**; residency-ordered visits (A4); `chunkPressure = paintStarve ∧ liveScouts>0`; warm-ring + **stall forensics** telemetry (`warm-ring-probe`, `stall-forensics`, `stalled-live-probe`). User hypothesis (“can’t enter next square”) = cold map chunks + captureStall, not entity blocked. Math: `N(R)≈π(R/64)²` → ~431 L0 at 750 blocks; observed ~358 ≈ radius ~683 blocks. Details: `docs/plans/login-bake-walltime-1033.md` § 1.0.38.
...
- Drop `distantvistas_1.0.38.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** `finished` climbs past **358**; `paintReadyQueued>0`; `chunkPressure:true` when starved; `captureStall` ≪ `residentPaint`/`painted`. **Scouts stay primary**; hop-unlock only if scout IO fails after fixes — see § Scouts primary; hop-unlock only if proven.
...
- Player still does not teleport. Exact pickup XYZ. Gray tent + black tips. No SIMD inside GetColor. No false-complete. Look lock intact.
- Drop `distantvistas_1.0.35.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** H-PAINT resumes after WaitChunks pile-up (`painted` / `partialPaint` / `waitExpirePaint`, not maxWait-only); first-30s gen0 much lower; spawn neighbourhood FlagBaked before rim; no star jumps.

## 1.0.34
- **Overlay scout stall fix (~358 L0 plateau).** Playtest `H-SCOUT-SEQ`: `heldNear` hundreds + `paintReadyQueued=0` while 16 scouts dwelled 80/400 ticks — near/far slot split starved FIFO; `maxWait` dropped keys without paint handoff. 1.0.34: **16-scout FIFO** (flush legacy held queues); **capture→paint ≤16 ticks** (`MaxCaptureWaitTicks` 80→16); partial capture handoff; `maxWait` 400→120 with re-queue + `partialPaint`. `WaitForMesh` is telemetry-only; spawn mesh gate stays overlay end. **Load-screen look lock:** `HoldLook` every overlay tick, server controls blocked, silent delta drain while locked (fixes interval star jitter). Background `PlayModeBakeBudget` unchanged. Details: `docs/plans/login-bake-walltime-1033.md` § 1.0.34.
- Player still does not teleport. Exact pickup XYZ. Gray tent + black tips. No SIMD inside GetColor. No false-complete.
- Drop `distantvistas_1.0.34.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** `scout-budget` shows `paintReadyQueued>0` most seconds; `heldNear≈0`; L0 climbs past 358 without interval stalls; no load-screen star jumps.

## 1.0.33
- **Login bake wall-time (GetColor throughput).** 1.0.32 parallel scouts still spent ~23 min in scalar `Block.GetColor`: up to 4096 columns × 16 stack layers per L0. 1.0.33 cuts invocations, not scout count: **per-section GetColor cache** (8×8 climate tile + blockId + Y), **stack early exit** when snow/water/canopy/autumn plants keep the top sample (`StackDeterminedByTopOnly`), **skip texture mean** outside deep-winter camouflage (`NeedsTextureMean`). Overlay paint uses **time-budgeted** `BakeSectionFromVisitChunked` (120 ms/tick cap, partial L0 resume) instead of one synchronous full section per stop. SQLite persist batches per paint tick. Stabilize holds **2** median windows (was 4) and releases when spawn is solid + far canvas ready even if horizon meshes still drain — no holes at spawn. Expected typical **1680 L0** cold login: **~2–3 min** wall (stretch **~90s**). Math + rejected ideas: `docs/plans/login-bake-walltime-1033.md`.
- Player still does not teleport. Exact pickup XYZ. Gray tent + black tips. No SIMD inside GetColor. No false-complete.
- Drop `distantvistas_1.0.33.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** overlay % moves every few seconds; ETA in minutes not ~23m; 16/16 scouts; spawn solid; same X/Y/Z as pickup.

## 1.0.32
- **Login bake speed (A-tier) plus GetColor GC cuts (B-tier) plus post-GetColor SIMD.** 1.0.31 already runs 16 scouts in parallel (8 near mesh-wait / 8 far FlagBaked-release) and paints `scoutReady` each tick instead of one serial `BakeBatchAtStop`. 1.0.32 folds the research-branch micro-wins and the remaining ranked bottlenecks: thread-local `LodBakeScratch` `BlockPos` + `ArrayPool` column arrays, **per-section texture-mean cache** (one 8-sample `GetColorWithoutTint` per BlockId per L0, not ×4096 columns), reused scout `readyScratch` / in-place leftover batch keys. Mix / halo / blur scratch also rent from `ArrayPool`. `MaxBakePerTick` stays **24**. Pinned scouts re-partition every **8** ticks (not every tick). Spawn reveal ring every **4** ticks. Mesh/Paint never `forceRecapture`. Sticky empty-mesh claims release the scout slot. Leftover neighbour bake is visit-cell-only past 1024. Overlay ETA uses wall/finishes (a 24-scout tick is not one 0.25s hop). Farseer visit-mask rebuilds at most every 500ms on HasDataSet churn (stamp L0 keys + envelope disk, not 256×256 `Contains`). Height enrich waits until overlay ends. **`LodRgbSimd` is live:** AVX2 then `Vector128` (SSE2/NEON) then scalar on `BlurLand`, `Quantize`, and RGB pack/unpack *after* GetColor — not inside `Block.GetColor`. Results are bit-identical to the scalar kernels (`BlurRadius` stays 0). Plan + citations: `docs/plans/login-bake-efficiency.md` and `docs/plans/simd-after-getcolor.md`.
- **Neglected-issue follow-up (audit `cursor/neglected-issues-audit-1ef2`).** In-window skip no longer claims “complete” with large holes: `FindMisses` or `LastUnfilledGaps` above **32** forces a scout fill; below that, skip names deferred leftover counts (frontier drip). Skip does not `ExploreBake.Clear()`. Frontier `MaxExplorePendingYield` is **24** (was 4) so 16-scout waves do not stall post-login fill. Load-time no-colour palette fill persists without a `MarkChanged` neighbor/mip storm (at most 2 remeshes/tick). Month-expire leftover GetColor skips `IsMissingTextureWhite` samples when maps never arrived. `CollectExpireLeftovers` keeps the nearest **256** L0 only. Leave-world `ClearMeshes` clears the mesh-pressure latch.
- **User-mentioned P0 (audit `cursor/user-mentioned-unfixed-8587`).** When FlagBaked meshes lag Farseer onset by more than 256 blocks, visit-onset uniforms pull smoke to the meshed rim (overlap 128) so the join is gray tent, not a white sky strip; onset recedes to 4.5× VD + 700 as meshes catch up. `region.fsh` unchanged. Parent coverage / gap-fill / eviction use `HasDrawableMesh` so sticky `emptyMeshKeys` cannot fake land. Overlay far-ready **75%** gate unchanged.
...
- **Verify:** overlay shows 16/16 scouts within seconds and % moves every few seconds; after overlay, no holes underfoot; FlagBaked meets Farseer as gray tent + black tips (no white sky strip); far land fills in; FPS back; same X/Y/Z as pickup.
...
- **Full despawn on teardown.** Overlay end / Esc / fail / leave `Die`s every scout viewer, `DespawnEntity` on the server, and UnloadChunkColumn the keep-loaded neighbourhoods (never the land under your feet). Overlay also restores the graphics slider **and** OverdrawStart (no leftover 750-hold / close overdraw eating FPS).
- **Cheaper than hops, same FlagBaked disk.** 6 concurrent viewers (not 16 far tessellation centers). Vanilla stream around you stays at the 750-hold so spawn is solid; scouts stream a local neighbourhood at each visit cell. Visit plan is still Farseer onset +700 (4075). No force-recapture of the whole disk every tick. Spawn-disk capture and scout rings use **separate sweep cursors** (sharing one cursor skipped spawn-local rows and left holes). Mip drain / visit paint **keep GPU meshes until swap-in** (the old dispose-first path punched dark holes and flashed).
- **Denser coverage in the same ~7 min wall.** Visit budget 480–1680 stops at 0.25s/stop fallback (hop-era 0.5s samples no longer cap the plan).
- **Login snapshot waits for a solid, centered world.** Overlay stays up until spawn-local L0 (1024 blocks) has drawable meshes and far meshes reach 75% of Farseer onset (~3056 of 4075). Mesh schedule/upload runs under the splash. Hold up to 90s after drain.
- Visit queue is **spawn-first**, then the rim, ordered near-to-far. 1.0.29 +700 / 4075 disk goal kept.
- **Less near-ground smoke.** Farseer gray tent + black tips stay (`inkAmt` cap 0.24); low-ground LOD mist and low Farseer smoke are thinner so they do not wash terrain or flash up close.
- Drop `distantvistas_1.0.30.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
- **Verify:** overlay does not move you (Pos stays at pickup). After overlay (or Esc/fail), same X/Y/Z + look as pickup; FPS back (no leftover far centers, no 750-hold). No dark holes underfoot; land centered on spawn; Farseer gray/black silhouette; no thick white blob / tearing on near desert. Coverage denser/farther than the hop fill.
...
- **Verify:** after overlay, FlagBaked land meets the gray/black Farseer silhouette (readable skyline, not sky mush, not flattened empty black) with no big empty/white gap; coverage visibly larger than the thin ~720-mesh / ~180-drawn playtest; meshes >> 180 drawn; player not hopped around.
...
- **Farseer onset ~700 blocks farther** plus grayish smoke silhouette (more gray near/low, little black far/top). Keep misty look, not ink wall.
- **LOD low-ground mist** — subtle fog wash on valleys/flats only.
- **Tree tops:** canopy/leaves keep pure vanilla GetColor at crown Y; upper wall band inherits crown colour (CrownSideBlocks 2). Paint revision 9 remeshes without forced login teleport.
- Drop `distantvistas_1.0.25.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again. With Pause-on-Start installed: join → overlay/scout completes without removing PoS.

## 1.0.24
- **No paint-revision login teleport.** Already-visited canvas no longer forces a full login visit-teleport solely because PaintRevision bumped (empty-mesh remesh, foliage Leaves, onset). Prefer idle/in-place remesh and discover bake; season refresh stays on the existing ~30-day / PlanSeasonExpired path.
- **Drop leftover Esc-paused resume** when an in-window complete stamp would skip the overlay, so joining does not re-wedge on a paused paint-rev pass (e.g. 95 of 120 left).
- Keep 1.0.23 mist/Farseer look. Smallest safe change.
- Drop `distantvistas_1.0.24.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again. Join should enter play without a paint-rev teleport hang.

﻿## 1.0.23
- **Sticky empty-mesh remesh.** Empty-mesh claims clear when new columns land so midground can remesh instead of staying blank.
- **Visit radius reaches Farseer onset.** Login/sweep visit disk scales to HorizonDrawScale (late 4.5x), with a wider envelope pad so sparse hops still count as swept.
- **Foliage Leaves path.** Season foliage uses BlockMaterial.Leaves plus canopy/bush paths for pure GetColor + FlagFrost (paint revision 8).
- Drop `distantvistas_1.0.23.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again. Expect one season overlay pass on worlds baked under older paint revisions.

## 1.0.22
- Midground coverage / foliage path interim toward 1.0.23 (empty-mesh remesh + Leaves GetColor). Prefer 1.0.23.
- Drop `distantvistas_1.0.22.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.

## 1.0.21
- **Farseer look rolled back to 1.0.18.** Same mist wash, soft 0.92 shadow, and late 4.5x onset (no near-rim lean cloud, no early 4.40x onset). Season colour work from 1.0.19 stays.
- Drop `distantvistas_1.0.21.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.

## 1.0.20
- **LOD land meets the Farseer rim.** Horizon draw goes to 4.65x view distance; Farseer onset sits at 4.40x so FlagBaked plates cover the silhouette start. Visit envelope pad is wider so the sweep fills that band.
- **Mistier Farseer onset.** Stronger fog wash and soft gray at the near rim, heightmaps lean away from the player (less vertical black wall through trees), less hard ink.
- **Crown colour on upper wall faces.** Far LOD sides pick up the same top frost/canopy wash on the uppermost block of each wall so white tips and autumn crowns read from the side. Paint revision 7 remeshes older caches.
- Drop `distantvistas_1.0.20.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again. Expect one season overlay pass on worlds baked under older paint revisions.

## 1.0.19
- **Far LOD season colours match vanilla up close.** Mid-autumn keeps species orange/red/yellow on trees and berry bushes (no fake snow crowns). Frost only after late autumn when climate is near freezing; winter frosts sides and UP crowns; early spring remesh drops the December white sheet. Paint revision 6 forces one recapture of older FlagBaked RGB.
- Audit matrix (code): mid-autumn frost gate closed (WinterAmount under 0.75); late autumn/winter FlagFrost + mesher wash; bush GetColor path; L1/L2 FlagBaked/FlagFrost canopy mip unchanged. Play: full quit, then mid-autumn / late frost / winter / early spring vs near vanilla.
- Drop `distantvistas_1.0.19.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again. Expect one season overlay pass on worlds baked under older paint revisions.

## 1.0.18
- **Farseer silhouettes are mistier.** Less hard black, more fog/sky haze wash, with enough shadow left to read as hills. Overlay still clamps SkyTint so cranked values cannot bleach the heightmap into empty sky.
- Drop `distantvistas_1.0.18.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.

## 1.0.17
- **Farseer stays far again.** Early 1x silhouette onset is gone. Visited and unvisited both fade in at 4.5x view distance so midground is Distant Vistas, not a close Farseer ring.
- **Quiet frontier scout fills the midground.** After login, a background drip streams and season-bakes L0 between your capture rim and that late Farseer band. Hard caps and yield when capture/mesh queues are busy. Prefers chunk visibility without moving you; rare quiet hops only in singleplayer or on servers that run Distant Vistas assist.
- Incomplete FlagBaked midground gets the same visit GetColor bake (trees, grass, frost) as login/walk.
- Drop `distantvistas_1.0.17.zip` in Mods. Do not extract. Fully quit Vintage Story, then start it again.
...
## 1.0.6
- **Farseer starts at our last draw circle (3Ã— view distance), not inside our LOD.** Stock Farseer began at 0.785Ã— view distance and tore mountains inside vanilla and Distant Vistas meshes. With Farseer installed we rewrite its region shaders: silhouettes begin at the same 3Ã— handoff where our lead-cone land drops off (`HorizonDrawScale`). Inside that circle is ours. Past it, unvisited far land is still Farseer's. Also killed the near Y-sink trench. Keep Farseer's Far View Distance past about 3Ã— your graphics view distance so the outer rim still has room.
...
## 1.0.3
- **Login hop-scan covers twice the linear radius again.** First-join / season-expire probe disk is ~144 km around you (`EmptyCanvasBootstrapRadiusBlocks` 72000 â†’ 144000). Same visit-stop wall budget; stops spread farther. MP skip gate and LeaveWorld re-init unchanged.
...
## 1.0.2
- **Login hop-scan covers twice the linear radius.** First-join / season-expire probe disk is ~72 km around you (`EmptyCanvasBootstrapRadiusBlocks` 36000 â†’ 72000). Same visit-stop wall budget; stops spread farther. MP skip gate and LeaveWorld re-init unchanged.
...
## 0.8.43
- **CRITICAL: GL_INVALID_OPERATION at SwapBuffers on new-world character create (0.8.42 playtest).** Overlay never opened. 0.8.42 proved vsvaogc hitch Drain was skipped (`join quiet` at 19:17:50). Remaining sequence: High Clouds/vanilla reloads `cloudmap` on LevelFinalize, then the first RunningGame present (character UI, no 3D rebind) hits glfwSwapBuffers with that FBO still bound â€” NVIDIA reports **The required buffer is missing** (GL_BACK does not exist on an FBO). Fix: bind the default framebuffer at the end of `OnLevelFinalize` (`CurrentFrameBuffer = null`; not OrthoMode / ClearFrameBuffer). Compile lodterrain and register the Opaque renderer only after character UI (`EnsureJoinRenderer`). Ship with vsvaogc 1.1.7. HUD overlay and delayed Open from 0.8.41 retained.
...
## 0.8.37
- **CRITICAL: join crash with TrueScale HD (access violation in GL.BindTexture during atlas compose).** Login splash created/bound textures from `LevelFinalize`, Harmony `OnNewFrame` Postfix, and LoadingGame present while vanilla `ComposeTextureAtlasses_StageB` still ran `BindTexture` on worker threads â†’ hard AV (`0xc0000005`). Fix: gate **all** splash GL until three `FinaliseTextureAtlas_StageC` passes complete (block/item/entity); keep vanilla loader visible until safe; defer `EnsureRunningGameRenderPath` and remove `PrepareImmediate` from LevelFinalize; drop `ApplyZFar` from LevelFinalize (0.8.9 regression). Purple explore FlagBaked bake + parent invalidate retained.

## 0.8.36
- **CRITICAL: lavender snap-back + magenta section patches (May/spring, high altitude / map view).** Hard chunky LOD-section rectangles and purple **grain** on map/overhead view = live-tint coarse meshes (shader `valuenoise` on band 0â€“2) and stale **parent L1+** GPU meshes after L0 visit bake. **Product intent:** login visit bake = live seasonal `GetColor` (30-day window + bootstrap). Explore uses the **same** `BakeSectionFromVisit`. **Fixes:** band 3 for all visit-baked rows; per-column visit bake; `InvalidateMipAncestors` + mip `onParentRemipped` remesh; disk L0 live-tint queues visit bake (not shader repro). Spring snow-row safety on unbaked grass only.

## 0.8.35
- **Login sweep retarget: ~1 minute wall-clock (was 3â€“5 min).** `TargetMaxSec` / `BootstrapTargetMaxSec` **60s**; `InitialSecPerStop` **2.0** â†’ `VisitStopBudget` yields **~30 land/coast teleports** (was 75 @ 300s). Efficiency: `TeleportSettle`/`BakeSettle` tightened; `MaxChunkWaitTicks` 48â†’**20** (~1.0s), `MaxCaptureWaitTicks` 28â†’**14** (~0.7s); batch neighbour bake radius **8** / cap **24** L0 cells per stop at **2048** view; open-ocean full-bake samples **3** (stamp fill unchanged). View boost floor **`SweepMinViewDistanceBlocks` = 2000** (ceiling 2048) held for the whole sweep via `EnsureBoosted` each tick. Gap-first `FilterNeedsVisit`, ocean stamp-not-full-bake, pose restore, and **0.8.34 splash opaque-cover fix** retained.

## 0.8.34
- **CRITICAL: opaque cover draw failed at valid frame size (0.8.33 playtest).** Present-path splash paint reached `DrawOpaqueCover` at 2560Ã—1369 but never logged `first opaque frame painted`. Root cause: `PaintSweepFrame` used `orthoPass: true` + internal GUI `Render2DTexture(int,â€¦, Vec4f)` â€” that tint path only works inside `EnumRenderStage.Ortho`. Harmony present hooks run outside the ortho stage, so internal quads failed; explicit `MeshRef` quads could also fail with depth test still enabled and `CurrentFrameBuffer` null (no `ClearFrameBuffer`). Fix: `PaintPresentPath` uses `orthoPass: false`, pairs `OrthoMode`/`PerspectiveMode`, calls `GLDisableDepthTest`/`GLEnableDepthTest` around the draw, resolves Primary framebuffer when `CurrentFrameBuffer` is null, and draws baked solid-color explicit quads before white/internal fallbacks. Register splash on `EnumRenderStage.Done`; LoadingGame present paints in Prefix when vanilla draw is skipped. 0.8.27â€“0.8.33 bootstrap/ocean/gap-first/pose-restore retained.

## 0.8.33
- **CRITICAL: black screen after char-create â†’ bootstrap (0.8.28 playtest).** RunningGame `RenderToDefaultFramebuffer` Postfix alone was insufficient: Harmony skipped `GuiScreenLoadingGame.RenderToDefaultFramebuffer` during `SweepActive` with **no replacement paint**, which prevented Ortho/AfterFinal render stages from running (`0 meshes`, `draw submit 0.0`, never `first opaque frame painted`). Handover was also deferred whenever login-sweep was **enabled in config**, not only while armed. Fix: paint splash on **LoadingGame + RunningGame present Postfixes** and **OnNewFrame Postfix** safety path; defer handover only while `SweepActive`; `EnsureRunningGameRenderPath` completes handover to RunningGame without clearing sweep; Ortho renderer stays active while sweep armed. Splash paint failure logs frame-size/cover diagnostics. 0.8.27â€“0.8.32 bootstrap/ocean/gap-first/pose-restore retained.

## 0.8.32
- **Login sweep always restores pre-sweep pose.** Capture position + look (yaw/pitch + camera) at `Begin()` immediately after character creation â€” before bootstrap teleports. `RestorePlayerPose` now applies `LockPlayerCamera` on every release path (success, Esc, abort, error, dispose). Teardown runs pose restore + `worldHide.Restore` in `finally` after view-boost/game-mode/HUD teardown so the player is never left at the last teleport stop. Resume snapshots persist camera XYZ for paused sweeps.
...
## 0.8.30
- **Bootstrap ocean: expose + sample bake + stamp fill (0.8.29 correction).** Open-ocean L0 body tiles no longer get a full teleport+bake each (0.8.29 skipped them entirely â†’ sky holes over water). Fix: up to **6 representative ocean sample visits** (spread across the body, not counted in the **75** land/coast stop budget) establish water palette from live terrain; remaining open-ocean cells get a **cheap flat stamp** (`LodLoginSweepOceanFill.StampOpenOcean`) after the visit pass so far vistas show water. Land, unknown, and coastline ocean cells still get full visit+bake. `InitialSecPerStop` restored to **4.0** â†’ **~75 land stops / ~5 min** (same 300s wall-clock cap). 0.8.27 splash present path, 2048 view boost, inner-weighted bootstrap, batch neighbour bake, and per-world complete retained.

## 0.8.29
- **Bootstrap ocean skip + bake efficiency (0.8.28 playtest follow-up).** Open-ocean L0 tiles (no land neighbour) are excluded from first-join planning â€” budget goes to land + coastline only (`SelectVisitWorthyCells`; coast-guard no longer queues every ocean body cell). Per-stop dwell tightened: TeleportSettle 10â†’4 ticks, BakeSettle 12â†’3, MaxChunkWait 80â†’48, MaxCaptureWait 48â†’28 (~1.4â€“2.4s caps). After capture idles at a stop, **batch-bake** up to 16 planned L0 neighbours already loaded by the 2048-block column sweep (6-cell radius) instead of one bake per teleport. `InitialSecPerStop` 4â†’2.5 â†’ **~100 visit stops / ~4 min** at estimate (same 300s wall-clock cap). View boost runs before the ARMED log so the first teleport sees 2048-block sweep. 0.8.27 splash + 0.8.28 inner-weighted bootstrap retained.

## 0.8.28
- **CRITICAL: first-join bootstrap too sparse (0.8.26 playtest).** `Bootstrap (new world) (38 of 27604)` finished in ~2.5 min but exposed only a tiny fringe beyond the walked frontier â€” 38 linear distance picks across a ~6 km / ~27k L0 probe disk is ~1 stop per long outer arc. Root cause: `BootstrapTargetMaxSec` was 150s (half the revisit window) and `BudgetVisitStops` sampled uniformly by distance rank. Fix: align bootstrap wall-clock with revisit (**300s / ~75 stops at 4s/stop**, same 3â€“5 min target); add `BudgetBootstrapVisitStops` â€” inner-weighted distance bands (~10â€“18 rings) with more teleports near spawn and multiple angular picks per ring so the full `EmptyCanvasBootstrapRadiusBlocks` disk gets usable vista cover. Revisit keeps linear `BudgetVisitStops`. 0.8.27 splash present path unchanged.

## 0.8.27
- **CRITICAL: black screen during login visit sweep (0.8.26 playtest).** `SuppressRunningGameRender=false` (0.8.25) let `RenderToPrimary` present, but registered Ortho/AfterFinal splash IRenderers never ran while `handOverRenderingToRunningGame` was deferred â€” `HasEverPaintedOpaque` stayed false, logs repeated `loading cover not painted yet`, player saw permanent black through bootstrap teleports. Root cause: splash paint was removed from the RunningGame **framebuffer present path** without a working substitute on the deferred-handover render graph. Fix: restore `PaintSplashCover` â†’ `PaintSweepFrame` on `GuiScreenRunningGame.RenderToDefaultFramebuffer` **Postfix** when `SweepActive` (not from OnNewFrame â€” that path does not present). Keep `SuppressRunningGameRender=false`, WorldHide, bootstrap-first planning (0.8.26), per-world complete, â‰¤30-day skip, OrthoMode/PerspectiveMode pair, and async-sound LoadingGame skip unchanged.
- **HUD restore after sweep.** `LodLoginBakeHudHide` retries hide/show up to three times, tracks when the sweep forced hide, and prefers showing HUD when restore is ambiguous â€” reduces `HUD restore mismatch (HideGuis=True, wanted=False)` after sweep finish.

## 0.8.26
- **CRITICAL: first login refresh-only left vistas as sky (0.8.24/0.8.25 playtest).** After the black-screen sweep finished `Refreshing visited land (season) (75 of 123)` with gate reason `no successful sweep recorded yet for this world`, the player saw no new land beyond what they had already walked. Root cause: `PlanSweepQueue` / `LodLoginSweepBootstrap.Plan` treated any non-empty `VisitedL0Keys` as a season revisit â€” so a few walked cells before the first successful sweep selected **PlanRevisitKeys** instead of the ~6 km bootstrap disk (`EmptyCanvasBootstrapRadiusBlocks`). Fix: when there is **no per-world complete marker**, always plan **Bootstrap** (coast guard / spawn-radius) with spatially spread `BudgetVisitStops` â€” even if visited keys are non-empty. Season **Refreshing visited land** only after a prior complete (miss-repair, canvas growth, or outside 30-day window). Gate orders no-complete before miss-repair. 0.8.25 splash present path retained (`SuppressRunningGameRender=false`, Ortho/AfterFinal splash, WorldHide, per-world complete, â‰¤30-day skip).

## 0.8.25
- **CRITICAL: black screen during login visit sweep (0.8.24 playtest).** `LodLoginBakeSweepGate.Arm` set `SuppressRunningGameRender=true`, so Harmony skipped `GuiScreenRunningGame.RenderToPrimary` / post / Ortho / AfterFinalComposition. OnNewFrame `PaintSweepFrame` cleared/painted with OrthoMode but nothing presented the framebuffer â€” teleports ran while the player saw permanent black (`loading cover not painted yet`, never `first opaque frame painted`). Fix: **do not suppress RunningGame render** during the sweep; keep WorldHide for teleport flash; splash draws only from registered Ortho/AfterFinalComposition IRenderers so present works. Gut OnNewFrame / framebuffer-prefix OrthoMode paint path (matrix-leak risk + no present). LoadingGame async-sound skip, char-create deferral, per-world complete (0.8.24), and â‰¤30-day skip unchanged.

## 0.8.24
- **CRITICAL: cross-world login skip (0.8.23 playtest).** `login-sweep-complete.json` was a single global file under `ModData/distantvistas/`. Gate compared `VisitedKeyCount` only, so World A's complete marker (e.g. 25 keys) made a new World B with a fresh LOD `.db` and fewer visited keys wrongly skip (`visited canvas complete within 30-day window`). Fix: scope complete + resume markers per world using the same `SavegameIdentifier` key as the LOD cache (`login-sweep-complete-<worldId>.json` / `login-sweep-resume-<worldId>.json`, plus `WorldId` in JSON). Gate never skips when WorldId does not match the current world. Legacy global files without a matching WorldId / schema 2 are ignored (safest). Rejoin same world within 30 days still skips. OrthoMode/PerspectiveMode restore and 30-day-only window retained from 0.8.23.
## 0.8.23
- **CRITICAL: Stack matrix overflow during login sweep (0.8.22 playtest).** `PaintSweepFrame` called `IRenderAPI.OrthoMode` every `ScreenManager.OnNewFrame` Prefix while the visit sweep was active. VS `ClientMain.OrthoMode` `GlPushMatrix`es the projection stack; without a matching `PerspectiveMode` (~32 frames) the stack fills, then `SystemRenderShadowMap.PrepareForShadowRendering` overflows on `StackMatrix4.Push` (black screen during post-char RunningGame teleports when splash painted with a polluted GL state). Fix: every splash paint that enters OrthoMode restores with `PerspectiveMode` in a `finally` block. OnNewFrame splash cover retained (still needed while `RenderToPrimary` is suppressed); LoadingGame async-sound skip unchanged.
- **Skip window is 30 in-game days only.** `LodLoginSweepWindow.IsWithin` no longer treats same-season alone as eligible â€” rejoin skip / resume requires `TotalDays` gap â‰¤ `MaxResumeDayGap` (30). Outside that window, re-sweep even if the season slug matches.

## 0.8.22
- **CRITICAL: rejoin Loadingâ€¦ deadlock (0.8.21).** `LodLoginBakeCharacterWait.IsPending` gated on `!PlayerReadyFired`, which is true on almost every `LevelFinalize` before handover â€” so rejoins deferred the sweep forever while Harmony still blocked `handOverRenderingToRunningGame`. Fix: `IsPending` is dialog-only (`HasProtectedDialogOpen`). LevelFinalize prefers Decide/skip first; real character/class deferral immediately allows handover (`AllowHandoverWhileCharacterPending`) so vanilla UI can show, then starts the sweep once dialogs close.
- **In-window skip ignores leftover incompletes.** If `login-sweep-complete.json` exists, visited count has not grown, and the season/30-day window still holds (`MaxResumeDayGap` = 30), skip the full revisit even when `FindMisses` reports leftovers â€” do not re-canvas the same world in-window. Empty-canvas bootstrap, eligible mid-sweep resume, canvas growth, and outside-window still run.

## 0.8.21
- **New-world first login deferral.** Login visit sweep no longer starts on `LevelFinalize` while character creation or class selection is open. `LodLoginBakeCharacterWait` polls until `PlayerReadyFired` and no character/class dialogs remain, then runs the empty-canvas bootstrap. `CloseBlockingDialogs` never closes those dialogs.
- **Login sweep time freeze hardened (0.8.20 playtest).** World time kept advancing during the visit sweep. `LodLoginBakeTimeFreeze` now anchors `TotalHours` on arm, forces `CalendarSpeedMul = 0`, and cancels net `SpeedOfTime` every tick. `Restore` removes the DV modifier, restores `CalendarSpeedMul`, clears the anchor, and logs `time restored` â€” each step independent so partial teardown cannot leave time frozen. `ReleaseResources` calls restore in a `finally` block on success, Esc, cancel, abort, and world leave.
...
- **0.8.20 retained:** splash painted from `OnNewFrame` every sweep frame, 3â€“5 min revisit budget, view-distance boost, miss-resweep cap, and deadlock fixes.

## 0.8.20
- **CRITICAL: login splash black screen (0.8.19 playtest).** Sweep ticks advanced from `ScreenManager.OnNewFrame`, but splash paint only ran on `GuiScreenRunningGame.RenderToDefaultFramebuffer` Prefix â€” that present path often never runs while world draws are suppressed, so `HasEverPaintedOpaque` stayed false and the screen stayed black. Fix: paint the DV splash every frame from `OnNewFrame` / `RenderPulse` while the sweep is active (framebuffer-prefix backup retained). Vanilla chunk hide is deferred until the first opaque splash frame. Miss-retry cap, 3â€“5 min revisit budget, view-distance boost, and deadlock fixes unchanged from 0.8.19.

## 0.8.19
- **Login visit sweep miss-retry cap (0.8.18 playtest).** After the budgeted main pass (~3â€“5 min revisit target), the audit phase retried the same ~12 incomplete regions for 4+ extra passes. `MaxResweepRounds` capped from 8 â†’ **1**; when the limit is hit the sweep calls `BeginDraining` and continues into play instead of looping. Splash opaque paint, 3â€“5 min revisit budget, and view-distance boost unchanged.

## 0.8.18
- **CRITICAL: login splash black screen (0.8.17 playtest).** While the sweep suppressed `GuiScreenRunningGame` world draws, Harmony also skipped `RenderToDefaultFramebuffer`, so nothing reached the screen and `HasEverPaintedOpaque` stayed false (`loading cover not painted yet`). Fix: paint every frame **before** RunningGame framebuffer present via `OrthoMode` + `ClearFrameBuffer` hard fallback + explicit full-screen quad, then backdrop/title/bar on top; allow framebuffer present (still skip world/post passes). Loading-game async-sound bypass, 3â€“5 min revisit budget, view-distance boost, and deadlock fixes unchanged.

## 0.8.17
- **Tighter login visit sweep (0.8.16 follow-up).** Season revisit target window **~3â€“5 minutes** (`TargetMinSec` 180, `TargetMaxSec` 300). `RevisitMaxVisitStops` = **75** at ~4s/stop (60â€“100 range depending on measured stop rate; `MaxVisitStops` ceiling 100). Example: 291 regions â†’ `Refreshing visited land (season) (75 of 291)` â‰ˆ **5 min**. Bootstrap (~2.5 min / 38 stops), view-distance boost+restore, splash, deadlock fixes, oversized resume replan unchanged from 0.8.16.

## 0.8.16
- **CRITICAL: login visit sweep wall-clock budget (0.8.15 playtest).** Season revisit with 291 visited L0 regions queued every key (~20s/stop from `MaxChunkWaitTicks=400`) â€” hours, not minutes. Fix: hard **â‰¤10 minute** revisit budget (`RevisitMaxVisitStops` = 150 at ~4s/stop, spatial subsample across the canvas like bootstrap). Bootstrap stays on its shorter ~2.5 min cap (`BootstrapMaxVisitStops` = 38). UI shows `Refreshing visited land (season) (150 of 291)` when subsampled.
- **Faster per-stop waits.** `MaxChunkWaitTicks` 400â†’80 (~4s max), `MaxCaptureWaitTicks` 120â†’48 (~2.4s), teleport/bake settle ticks trimmed. Enough time to capture live terrain; typical stop ~4s instead of ~20s.
- **Oversized resume reclaim.** Pre-0.8.16 checkpoints with 280+ pending revisit keys are discarded on load and replanned into the new budget â€” Esc cancel/resume still works within the cap. Incomplete-only repair passes are unchanged (never subsampled).
- **View-distance boost during sweep.** Temporarily raises vanilla `DesiredViewDistance` to the server-approved max (up to 2048 blocks), clears `FarViewDistanceCap` for unlimited DV draw, and lowers `OverdrawStart` to 0.35 for wider cover. Chunk stream/sweep radius scales with boosted view distance. All settings restore on finish, Esc, or cancel â€” play FPS unchanged.
- **0.8.14â€“0.8.15 retained:** async-sound deadlock fix, full DV splash (backdrop+title+bar), Esc cancel/resume.

## 0.8.15
- **Login visit sweep splash restored (0.8.14 regression).** The running-game overlay again paints the full Distant Vistas join screen every visit: landscape backdrop, arched title art, progress bar, and Preparing/Visiting/Finishing status â€” not the bare black stock-only cover from 0.8.14. Ortho draw now prefers the proven internal-quad tint path before explicit `MeshRef` quads so `HasEverPaintedOpaque` cannot silently stay false while the world is hidden. 0.8.14 deadlock fixes retained: Harmony still skips `GuiScreenLoadingGame.RenderToDefaultFramebuffer`, sweep stays on `GuiScreenRunningGame`, and pulse still runs from `ScreenManager.OnNewFrame` before any screen draw.

## 0.8.14
- **CRITICAL: login visit sweep stuck at Preparing 1% (0.8.13).** While handover was deferred the client stayed on `GuiScreenLoadingGame`; `ScreenManager.Render` blocked in `RenderToDefaultFramebuffer` waiting for async sounds, so `LodLoginBakePulse` never ran and warmup never reached `warmup complete â€” entering visit teleports`. Fix: Harmony skips vanilla loading-screen draw during sweep; sweep immediately switches to `GuiScreenRunningGame` (world draws still suppressed) and paints the stock Loadingâ€¦ cover; pulse runs from `ScreenManager.OnNewFrame` **before** any screen draw as well as from the overlay renderer.

## 0.8.13
- **CRITICAL: login visit sweep skipped forever (0.8.12).** `LodLoginSweepGate` returned skip when the visited canvas had no audit misses â€” even when `login-sweep-complete.json` was missing â€” and the skip path called `RecordSuccess`, permanently stamping a fake completion. Fix: run sweep when no prior successful sweep is recorded; only skip when the complete marker exists, visited count has not grown, still within the season/day window, and audit finds no misses. `RecordSuccess` is written only when a sweep actually finishes (`LodLoginBake.ReleaseResources`).

## 0.8.12
- **CRITICAL: double-assembly load / missing DistantVistas (0.8.11).** `clientsettings.json` `modPaths` listed both relative `Mods` and the absolute AppData `Mods` folder â€” the same zip was discovered twice. First `LoadAssembly` succeeded; the second failed with `Assembly with same name is already loaded`, the failed container poisoned the mod (no External Origins entry, no systems, no `DistantVistas 0.8.11 loaded` line), and the client stayed on vanilla `Loadingâ€¦` at `Loading world map cache db...`. Fix for install: keep a **single** modPath to AppData `Mods`. Zip install only there; never also under `ClientMods` as a live origin.
- **Handover release hardened.** `ClearHandoverDeferral` clears sweep/deferral flags **before** screen attach, always best-effort sets `loadingText` to `Finishing 100%`, and still forces `LoadScreenNoLoadCall(running)` when possible even if vanilla loader attach failed earlier. LevelFinalize wraps sweep setup in try/catch so a setup exception still releases Loadingâ€¦.
- **Visible progress on vanilla loader.** Status lines now use phase labels **Preparing / Visiting / Finishing** with an explicit **percent** (throttled every ~5%). Driven via `ScreenManager.loadingText` (GuiScreenLoadingGame exposes `textElem` only â€” no stock progress-bar field to hook). Custom DV splash stays off.

## 0.8.11
- **CRITICAL: stuck Loadingâ€¦ after sweep skip (0.8.10).** Harmony blocked `handOverRenderingToRunningGame` whenever sweep was enabled in config â€” including when `CompleteHandover` re-invoked it on the skip path. The client logged success but never left `GuiScreenLoadingGame`. Fix: `AllowHandoverPassThrough` bypass during completion; `ClearHandoverDeferral` runs vanilla handover **and** `LoadScreenNoLoadCall(running)` on every exit (skip, success, cancel, abort, leave). Loud log: `handover deferral cleared â€” reason=skip|success|cancel|abort|â€¦`.
...
- **Sweep status heartbeat.** Writes `ModData/Logs/login-sweep-status.json` every 3s with phase, mode, regionsTotal/Done, lastAdvanceUtc, stuckHint (no progress &gt;10s).

## 0.8.5
- **CRITICAL: login visit sweep deadlock on vanilla loading hold (0.8.3â€“0.8.4).** Re-rendering `GuiScreenLoadingGame` stalled VS game-tick listeners, so warmup never advanced past `preparing loading screenâ€¦`, teleports never started, and Esc could not reach the cancel handler. Fix: drive sweep logic from a render-frame pulse (`LodLoginBakeRenderDriver`) at the same 50 ms cadence; poll Esc via `KeyboardKeyState` on every render frame; warmup proceeds after one pulse (no `HasRendered` gate). Loud logs: `warmup complete â€” entering visit teleports`, `quiet teleports begin`.
...
- **Vanilla hold: no `OnScreenLoaded` on cached screens.** Re-calling it on a cached `GuiScreenLoadingGame` re-triggered async sound/asset loading (client spammed `Waiting for async sound loadingâ€¦`). Only fresh instances call `OnScreenLoaded`. Merged 0.8.4 `CachedScreens` Typeâ†’GuiScreen lookup + `LoadAndCacheScreen` reflection.
...
## 0.8.3
- **Vanilla loading screen during visit sweep (product direction).** The custom Distant Vistas landscape splash and gold title panel are no longer the primary join UX. While the login visit sweep runs, the mod re-renders Vintage Story's native `GuiScreenLoadingGame` and updates `ScreenManager.loadingText` with sweep progress â€” the same stock "Loadingâ€¦" view you see when a world first comes in. Quiet teleports, per-column season bake, miss re-sweep, cancel/resume, and skip-if-complete gates are unchanged. Play releases only when the sweep finishes, is skipped, or you cancel.
- **Stock fallback only when vanilla cannot be held.** If `GuiScreenLoadingGame` cannot be resolved or drawn, a minimal dark fill + "Loadingâ€¦" + progress bar is used â€” not the DV backdrop/title splash. Log line: `stock Loadingâ€¦ fallback (vanilla screen unavailable)`.
- **No fade-out phase.** The vanilla screen simply hides on release (same as normal world load handoff).

## 0.8.2
- **CRITICAL: overlay health gate aborted sweep with no UI (0.8.1 regression).** 0.8.1 routed *all* draws through explicit `MeshRef` only; on many VS 1.22.7 joins that path never succeeds on the ortho pass, so `ConsecutiveOpaqueFrames` stayed at 0 and the sweep aborted after 200 ticks with no loading screen or teleports. Fix: dual draw path â€” explicit quad on `AfterFinalComposition` (NRE-safe), **ortho internal-quad tint fallback** (`Render2DTexture(int, â€¦, Vec4f)`) restored for the primary pass. Health gate no longer aborts the sweep when paint counter fails; logs a warning and continues after warmup timeout.
...
- **Viewport thrash fix.** The sweep now pins `Entity.CameraPos` (and clears `CameraPosOffset`) to the join pose while the entity quietly teleports for chunk streaming â€” the world no longer scrolls behind the loader. The full overlay (backdrop, title, panel) paints on **both** `Ortho` and `AfterFinalComposition` so sky/terrain cannot flash through between passes.
...
## 0.7.99
- **Login visit sweep OFF by default (safe build).** `LoginVisitSweepEnabled` in `distantvistas.json` defaults to `false` â€” restores **0.7.78-style immediate play** with no overlay, teleports, or join thrash. Set `true` or `VINTAGEHORIZONS_LOGIN_SWEEP=1` only when testing.
...
## 0.7.97
- **Dense season samples per L0 stop (schema v2).** Each visit stop now writes a `stop` header (section bounds, chunk columns, quadrant coverage, provisional flags, snow vote) plus **4096 `column` rows** â€” one per cell in the 64Ã—64 grid, including uncaptured gaps with coverage flags. Captured columns add block ids/codes/domain/path, RGB, palette flags, run geometry, subsurface block, canopy, snow stack, climate, and calendar fields.
- **Skip / resume / crash fix unchanged (0.7.96â€“0.7.95).** Complete canvas within season/30-day window skips the overlay; Esc cancel/resume; `LodLoginBakeInputLock` crash fix retained.

## 0.7.96
- **Skip sweep when canvas is already complete.** If every visited L0 region passes the miss audit and login is still the **same season** or within **30 in-game days** of the last successful sweep, the overlay is not shown â€” play starts immediately with persisted canvases. Sweep still runs for incomplete/missed regions, an expired season/day window, empty-canvas bootstrap, or resuming a cancelled mid-sweep checkpoint (`login-sweep-resume.json`). Successful sweeps record `ModData/distantvistas/login-sweep-complete.json` for the gate.
- **0.7.95 crash fix retained.** `LodLoginBakeInputLock` still blocks input via `StopAllMovement()` and a guarded action list â€” no raw `InWorld*MouseDown` enum iteration.
...
## 0.7.94
- **VS 1.22.5 API compile fixes.** `IRenderAPI.CameraType` and `IInputAPI.MouseGrabbed` are read-only â€” player hide uses transparent tint + input guard only; `IList<Block>` for `world.Blocks`; bootstrap usings; screen renderer float/ref locals; cairo-sharp reference for Cairo title fallback.

## 0.7.93
- **Quiet visit-sweep teleports.** Login bake moves the local player via client `EntityPos.SetPos` + `UpdatePartitioning` + `SetChunkColumnVisible` â€” no `/tp` chat command, no server command echo, no teleport audit spam. Pose still restores cleanly through the same quiet path on release.
...
## 0.7.91
- **Empty-canvas bootstrap (new world).** When no visited L0 keys exist, login sweep no longer uses a tiny timing-budget ring. It probes rain height near spawn: large ocean triggers **Bootstrap (coast guard)** â€” every ocean L0 cell in the bootstrap disk plus all land coastline neighbours; otherwise **Bootstrap (new world)** sweeps a ~6000-block-radius disk (`EmptyCanvasBootstrapRadiusBlocks`, tunable). Existing visited canvases still use **Revisiting visited land** (full visited L0 set, can exceed 6k). Progress UI and log lines name the active mode.

## 0.7.90
- **Per-column captured colours (no whole-cell flat fill).** Palette rows are keyed by block id **and** stored colour, capture registers each column at its own position, and `BakeSectionFromVisit` samples `GetColor` per column top â€” splitting palette rows when neighbours differ. Fixes the hard-line 64-block checkerboard / manila plates from one averaged colour per block id across the L0 cell.
...
- **Bootstrap timing + ETA.** Empty-cache bootstrap sizes a coast-biased L0 ring to ~1.5â€“2.5 min from measured capture rate; progress UI shows time remaining. Large existing visited canvases keep full sweep with honest ETA.
...
## 0.7.84
- **Login bake colour match (critical).** Visit-sweep bake now samples vanilla `Block.GetColor` at each column top (with split climate + season fallback matching the live shader). Baked sections use alpha band 3 so the mesher and shaders display stored RGB exactly â€” no manila wash from live climate/season multiply, keep-origin ratio, snow overlay, or valuenoise.

## 0.7.83
- **Login bake teardown.** One idempotent `Teardown()` path on success, abort, error, and world leave â€” hides overlay, restores player pose, unlocks controls/mouse, unmutes audio, and unfreezes calendar time with no leftover state.

## 0.7.82
- **Login sweep overlay + input lock (critical fix).** Layout locked: (1) full-screen landscape photo `assets/distantvistas/textures/gui/login-backdrop.png`, (2) centered arched solid-gold title graphic `assets/distantvistas/textures/gui/login-title-rainbow.png` (rainbow shape, single gold â€” not multicolor letters), (3) loading panel below with progress %, bar and status. Paints every frame via `IRenderer` at `EnumRenderStage.Ortho`; solid dark fill and gold arched text only if assets are missing from the zip.
...
## 0.7.81
- **Visit-sweep purpose locked.** Login pass exists to gather live season truth by being at each visited 64Ã—64 canvas: re-capture loaded voxels (snow on ground, snowy/part-green trees, autumn tone) while teleported there under the full-screen overlay, season-bake per column top, persist to SQLite, propagate to parent mips so near and far LOD match until relog. Not finalize-time recolor of unloaded cache.
...
- **Discarded:** immediate SQLite palette bake on `LevelFinalize` â€” cached rows lack current-season truth until chunks load and the player visits.
...
- **Discarded approach.** Discover-bake at capture plus budgeted month repaint (0.7.79 draft line) was dropped in favour of one login sweep â€” simpler truth model, no idle remesh storms when the calendar changes.

## 0.7.78
- **Turn hitch frame budgets.** Look-only (no 64-block origin shift) no longer walks or mesh-requests tiles entirely behind the camera and outside the 15Â° lead cone; GPU meshes stay pinned. Per-frame memo for lead cone, frustum-in-view, land-like, and captured-beyond probes cuts repeated sqrt/frustum work during the quadtree walk. RenderDirty prune spreads to every 4th frame while idle so prune + walk + schedule do not stack on every yaw tick. FOV occlusion cache is lazy per tile (~10Â° yaw slack per entry) instead of wiping the whole table on small turns. Lead-cone L0 promote cap 2/frame (was 3). Cake ban, occlusion fail-open, pressure-only eviction, and 0.7.77 hysteresis unchanged.

## 0.7.77
- **Pressure thrash hysteresis.** Enter mesh pressure only after ~1.5s of sustained bad frames (enter at ~40ms p95 / ~37ms avg), and clear only after ~2.5s under a lower bar (~25ms p95 and ~28ms avg). Stops the 0.7.75/0.7.76 feedback loop where whole-client ~33ms deltaTime permanently latched pressure, eviction/remesh made frames worse, and FPS bounced 15â€“48. Eviction under pressure capped at 2 meshes/frame with a short cooldown so eviction itself cannot spike walk/draw. Mesh count alone still never opens pressure. Telemetry: `pressureEnter`, `pressureClear`, `pressureActiveMs`. Cake ban, FOV occlusion, fail-open unchanged.
...
## 0.7.47
- **LOD stays off vanilla.** 0.7.45 drew captured L0/L1 from 0.55Ãƒâ€” view distance while vanilla still owns the ground out to 1.0Ãƒâ€”. A straddling L1/L2 painted its near half over loaded chunks: brown cubes on the tree in your face, stair-step fight on the hill, flicker on ice. The fragment shader now drops any LOD pixel still inside view distance (unless you are high enough that vanilla dropped that column). A refused parent next to you is not submitted. Foam-white water (atlas sparkle / missing-tex) is forced back to lake blue so streams are not ice. Isolated 0.7.47; still not a public push.
## 0.7.46
...
CapturedColumns=1024 (one 32x32 quadrant) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â meshed as filled rectangles with vertical
...
not descend ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â then hit `if (insideVanilla) return false` and drew nothing, so the ring
...
read as vertical slabs / isolated pillars. 0.7.6 never skips drawing for open sides ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â
...
`PatchVanillaEdgeFade` (default true); for this release the override is always packaged ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â
