...
- **Full despawn on teardown.** Overlay end / Esc / fail / leave `Die`s every scout viewer, `DespawnEntity` on the server, and UnloadChunkColumn the keep-loaded neighbourhoods (never the land under your feet). Nothing should keep meshing 4 km out afterward.
- **Denser coverage in the same ~7 min wall.** 16 concurrent viewers. Visit budget 480–1680 stops at 0.25s/stop fallback (hop-era 0.5s samples no longer cap the plan).
...
- **Verify:** overlay does not move you (Pos stays at pickup). After overlay (or Esc/fail), same X/Y/Z + look as pickup. No leftover far entities/centers. No dark holes underfoot; land centered on spawn; Farseer gray/black silhouette; no thick white blob on near desert. Coverage denser/farther than the hop fill.
