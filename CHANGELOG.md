## 0.8.36
- **CRITICAL: lavender/purple far LOD after leave (May/spring playtest).** Hard swap at vanilla load/unload — DV LOD mesh kept live-tint lavender; vanilla only covered it while loaded. Root cause: explore capture stored untinted composite + live tint (`DescribePalette` always `Baked=false`) so `albedo = vertexColor * tint`. **Primary fix:** `TryDiscoverBake` → `FlagBaked` + `SlotNone` (band 3): `GetColor` when map chunk resident, else snow-guarded `BakePaletteColor` (no chunk required for grass). `FinalizeL0DiscoverBake` on capture apply runs `BakeSectionFromVisit` + `UpgradeLegacyEntries`, **always** `InvalidateGpuMesh` + remesh, and mip burst so coarse parents match. Spring snow-row hypothesis: snow caps need resident `GetColor`; valley grass never live-tints snow white. Safety nets: `LodExploreBake` drain, `lodterrain.vsh` snow-row clamp + luminance climate. **Also:** splash cover-fit + tiled quads; login sweep ~60s at VD≈2000.
