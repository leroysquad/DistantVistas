## 0.7.81
- **Season bake survives disk reload (again).** `RefreshStoredPalette` ran `RefreshStable` on every palette row, including `FlagBaked` entries. That replaced discover-baked grass/leaf RGB with `StableColorOf` (atlas mean / dirt-heavy topsoil composite) while the baked flag and shader skip stayed, so walked land snapped to flat brown 64-block rectangles after walk-away or rejoin. Baked rows are now left alone; legacy live-tint rows still heal via `UpgradeLegacyEntries`.
- **Turn hitch: mesh schedule defers off-screen dirty keys.** `SelectNearestDirty` now applies the same look-only deferral as `RequestMesh`, so a small yaw no longer walks the entire `RenderDirty` set for tiles entirely behind the camera and outside the lead cone.
- **Server-assist foreign sections heal on install.** `InstallForeignBlob` now runs `AfterSectionLoaded` so legacy live-tint caches from network assist get discover-baked without a manual cache wipe.
- **Note:** A local-only **0.8.45** tip exists on the player's machine (not yet on GitHub). It targets greedy topsoil plates and denser climate; this 0.7.81 patch stays on the 0.7.80 policy tree. Rebase/compare once that branch is pushed.
...
## 0.7.79
- **Discover-bake seasonal colour.** New captures piggyback Vintage Story's own `ApplyColorMapOnRgba` climate + season maps at each block's X/Y/Z (same path vanilla grass and leaves use), multiply into the stored palette, and mark `FlagBaked` with tint slot 0 so walking away keeps the look you had nearby. Mountain foliage samples at its real height/climate, not a valley keep-origin. Snow and ice blocks stay untinted as before.
- **Gradual month repaint.** Calendar month change queues `SeasonDirty` sections and repaints baked palettes two sections per tick (plus `RenderDirty` remesh) — never all-at-once. Legacy untinted + live-slot caches keep the shader clock until revisited or repainted. Cold rows join the queue when loaded during a repaint epoch.
- **A/B validation (January, March, April).** Stand on walked ground with vanilla chunks loaded: note grass-top and leaf hue within ~1 chunk. Walk out until only LOD draws the same hillside; baked 0.7.79 land should match that hue instead of snapping grey-green or white. Change month with `/time` or sleep: far land should shift hue over several seconds without a frame hitch. Compare mountain canopy to grass on the same ridge — both should share the colder palette, not valley lime on peaks.
