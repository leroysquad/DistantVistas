## 1.0.38
- **Warm-ring cliff fix (~358 stall).** Root cause: filled-disk progress exits the overlay **stream-hold** radius (~750 blocks) while pending keys remain inside the **spawn-solid** band (1024) — classified as near scouts on **cold-near** cells with no map chunks. Forced Capture without residency → **captureStall** hot-loop (733 stalls vs 6 painted in 1.0.37); `chunkPressure` never fired because scouts were in Capture not WaitChunks. Fix: **paint-readiness scheduling** — never force Capture on cold keys while paint starves; resident handoff in Capture; exponential captureStall cooldown; cap cold-near live scouts to half fleet; residency-ordered visits (A4); `chunkPressure = paintStarve ∧ liveScouts>0`; warm-ring telemetry (`warm-ring-probe`). Math: `N(R)≈π(R/64)²` → ~431 L0 at 750 blocks; observed ~358 ≈ radius ~683 blocks. Details: `docs/plans/login-bake-walltime-1033.md` § 1.0.38.
